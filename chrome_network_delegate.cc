...
bool IsAccessAllowedChromeOS(const base::FilePath& path,
  ...
  // /run/imageloader is the root directory for all DLC packages. The
  // "timeofday" package specifically contains assets required for one of ash's
  // screen saver themes.
  allowlist.push_back(
...
void ChromeNetworkDelegate::OnURLLoaderFactoryCreated(
    mojo::PendingReceiver<network::mojom::URLLoaderFactory> factory_receiver,
    mojo::PendingRemote<network::mojom::URLLoaderFactory> target_factory) {
  // The CustomURLLoaderFactory will self-delete when the mojo connection is
  // terminated. This matches Chromium's common pattern for self-owned mojo
  // objects.
  new CustomURLLoaderFactory(std::move(factory_receiver),
